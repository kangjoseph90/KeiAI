...
describe('identityKey', () => {
	...
	describe('deriveSharedSecret', () => {
		...
		it('should derive different secrets for different key pairs', async () => {
			...
			const aliceCharlieSecret = await deriveSharedSecret(alice.privateKey, charlie.publicKey);
	...
	describe('full registration/login flow simulation', () => {
		it('should complete E2EE identity key storage roundtrip (register → login)', async () => {
			...
			const encryptedPrivate = await encrypt(masterKey, JSON.stringify(Array.from(rawPrivate)));
			rawPrivate.fill(0);
			...
			const secretRestoredWithBob = await deriveSharedSecret(restoredPrivateKey, bob.publicKey);
