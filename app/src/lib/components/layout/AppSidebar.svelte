				...
				{#each filteredCharacters as char (char.id)}
					...
					>
						...
						{#if isActive && editingId !== char.id}
							<div class="hidden group-hover:flex gap-0.5" onclick={(e) => e.stopPropagation()}>
								<Button
									variant="ghost"
									size="sm"
									class="h-5 w-5 p-0"
			...
			<div class="px-2 pb-2">
				...
				>
					<Avatar.Root class="size-7 shrink-0">
						...
						<Avatar.Fallback>{($activeUser?.name ?? 'U').charAt(0).toUpperCase()}</Avatar.Fallback>
					</Avatar.Root>
					<div class="flex flex-col overflow-hidden">
						<span class="text-xs font-medium truncate">{$activeUser?.name ?? 'Guest User'}</span>
						<span class="text-[10px] text-muted-foreground truncate">{$userEmail ?? 'Offline'}</span
						>
					</div>
				</div>
			</div>
