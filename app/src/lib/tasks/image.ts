...
export async function runImageGeneration(messageId: string): Promise<void> {
    if (getImageGenerationTask(messageId)?.status === 'generating') {
        throw new AppError('INVALID_INPUT', `Image generation is already running: ${messageId}`);
    }
    ...
    if (!source.trim()) throw new AppError('INVALID_INPUT', 'Image generation source is empty');
    ...
    try {
        ...
        for await (const output of runtime.run()) {
            ...
            finalIds = inlayIds.filter((id) => {
                ...
                return ref && getAssetMediaType(ref.mimeType) === 'image';
            });
            if (finalIds.length > 0) {
                await updateMessageSwipe(message.id, swipe.id, { imageAttachments: finalIds });
            }
        }
        if (finalIds.length === 0) {
