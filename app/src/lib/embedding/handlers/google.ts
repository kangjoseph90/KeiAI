...
export class GoogleEmbeddingHandler implements EmbeddingHandler {
	...
	async embed(texts: string[], signal?: AbortSignal): Promise<EmbeddingResult> {
		...
		const response = await appHttp.fetch(`${this.config.baseUrl}/${modelName}:batchEmbedContents`, {
			method: 'POST',
			headers: {
		...
		if (!json.embeddings || !Array.isArray(json.embeddings)) {
			throw new AppError('NETWORK_ERROR', 'Google Embedding returned invalid response structure');
		}
