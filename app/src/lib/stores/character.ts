...
export async function selectCharacter(characterId: string) {
	...
	if (!detail) {
		throw new Error(`Failed to select character: ${characterId} not found`);
	}
...
export async function updateCharacterSummary(
	...
	if (!updated) {
		throw new Error(`Failed to update summary for character ${characterId}`);
	}
...
export async function updateCharacterData(
	...
	if (!updated) {
		throw new Error(`Failed to update data for character ${characterId}`);
	}
