...
export class AssetService {
	...
	static async getAssetUrl(id: string): Promise<string | null> {
		...
		if (fields.remoteUrl) {
			...
			await AssetService.registerCache(id, decrypted.byteLength);
			return appStorage.getRenderUrl(id);
		}
	...
	static async evictCacheIfNeeded(): Promise<void> {
		const all = await localDB.getAll<CacheRegistryRecord>('cacheRegistry', '');
	...
	private static async registerCache(id: string, size: number): Promise<void> {
		const now = Date.now();
