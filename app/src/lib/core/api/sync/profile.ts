 ...
 *
 * Push: Called by ProfileStore after local writes.
...
import { ProfileService, type Profile } from '../../../services/profile.js';
...
export class ProfileSyncService {
	...
	static async pushProfile(name: string, avatarDataUri?: string): Promise<void> {
		...
		try {
			...
			// If PB assigned an avatar file, replace the heavy Base64 data URI
			// in local DB with the lighter PB file URL.
			if (record?.avatar) {
				const serverAvatarUrl = pb.files.getURL(record, record.avatar);
				await ProfileService.update({ avatar: serverAvatarUrl });
			}
		} catch (err) {
