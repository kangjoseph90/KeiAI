...
<main
	style="max-width: 800px; margin: 40px auto; font-family: sans-serif; display: flex; gap: 20px;"
>
	{#if errorMsg}
		<div
			style="background: red; color: white; padding: 10px; width: 100%; position: absolute; top: 0; left: 0;"
		>
			{errorMsg}
		</div>
	{/if}
	...
	{:else}
		<!-- Sidebar Navigation -->
		<nav
			style="width: 150px; display: flex; flex-direction: column; gap: 10px; border-right: 1px solid #ccc; padding-right: 20px;"
		>
			<h3 style="margin-top: 0;">Menu</h3>
			{#each ['characters', 'personas', 'presets', 'modules', 'plugins', 'settings'] as menuView}
				<button
					on:click={() => switchView(menuView as ViewMode)}
					style="padding: 10px; text-align: left; background: {view === menuView ||
					(menuView === 'characters' && (view === 'chats' || view === 'chat'))
						? '#007BFF'
						: '#f4f4f4'}; color: {view === menuView ||
					(menuView === 'characters' && (view === 'chats' || view === 'chat'))
						? 'white'
						: 'black'}; border: none; border-radius: 4px; cursor: pointer;"
				>
		...
		<div style="flex: 1;">
			...
			>
				<h2 style="margin: 0;">
					{#if view === 'characters'}
						Characters
					{:else if view === 'chats'}
						{$activeCharacter?.name}'s Detail & Chats
					{:else if view === 'chat'}
				...
				{#if view === 'chats' || view === 'chat'}
					<button
						on:click={() => switchView(view === 'chat' ? 'chats' : 'characters')}
						style="padding: 5px 15px;">⬅ Back</button
					>
				{/if}
			</div>
			...
			{#snippet renderEntityList(
				...
				<div style="display: flex; flex-direction: column; gap: 10px;">
					{#each items as item (item.id)}
						<div
							style="padding: 15px; background: #f4f4f4; border-radius: 8px; border: 1px solid #ddd;"
						>
									...
									<button
										on:click={() =>
											type === 'character'
												...
												: handleUpdate(type, item.id)}>Save</button
									>
								...
								<div style="display: flex; justify-content: space-between; align-items: center;">
									...
									<div style="display: flex; gap: 5px;">
										...
										<button
											...
											style="background: #ff4444; color: white;">Del</button
										>
									</div>
								</div>
							{/if}
			...
			{#if view === 'characters'}
				...
				<div style="display: flex; flex-direction: column; gap: 10px;">
					{#each $characters as char (char.id)}
						...
						<div
							style="padding: 15px; background: #f4f4f4; border-radius: 8px; border: 1px solid #ddd; cursor: pointer;"
			...
			{:else if view === 'presets'}
				{@render renderEntityList($promptPresets, 'preset', handleCreate, 'New Preset Name')}
			{:else if view === 'modules'}
			...
			{:else if view === 'settings'}
				...
				>
					<h3>App Settings</h3>
					<pre style="background: #eee; padding: 10px; overflow-x: auto;">{JSON.stringify(
							$appSettings,
							null,
							2
						)}</pre>
					...
					<div style="flex: 2;">
						...
						<div style="display: flex; flex-direction: column; gap: 10px;">
							{#each $chats as chat (chat.id)}
								...
								<!-- svelte-ignore a11y_no_static_element_interactions -->
								<div
									style="padding: 15px; background: #eef7ff; border-radius: 8px; cursor: pointer; border: 1px solid #bce8f1;"
			...
			{:else if view === 'chat'}
				<div style="display: flex; gap: 20px;">
					<!-- Left Col: Chatting -->
					<div
						style="flex: 2; background: #fafafa; border: 1px solid #eee; border-radius: 8px; height: 600px; display: flex; flex-direction: column;"
					>
						<!-- Messages Area -->
						<div
							style="flex: 1; overflow-y: auto; padding: 15px; display: flex; flex-direction: column; gap: 15px;"
						>
							{#each $messages as msg (msg.id)}
								<div
									style="align-self: {msg.role === 'user'
										? 'flex-end'
										: 'flex-start'}; max-width: 80%; width: 100%;"
								>
									<div
										style="display: flex; justify-content: {msg.role === 'user'
											? 'flex-end'
											: 'flex-start'}; gap: 5px; margin-bottom: 2px;"
									>
										{#if msg.role === 'user'}
											<button
												on:click={() => {
												...
												style="font-size: 0.7em; padding: 2px 5px;">Edit</button
											>
											<button
												on:click={() => handleDeleteMessage(msg.id)}
												style="font-size: 0.7em; padding: 2px 5px; background: #ff4444; color: white;"
												>Del</button
											>
										{/if}
									</div>
									...
									{:else}
										<div
											style="background: {msg.role === 'user'
												? '#007BFF'
												: '#E9ECEF'}; color: {msg.role === 'user'
												? '#FFF'
												: '#000'}; padding: 10px 14px; border-radius: 12px; display: inline-block;"
										>
											{msg.content}
										</div>
									{/if}
