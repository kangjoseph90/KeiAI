...
14. [Testing Conventions](#14-testing-conventions)
...
We use **Vitest** as the test runner with **happy-dom** for DOM simulation. Test files are co-located with source code or organized under `tests/unit/` for pure unit tests.
...
Crypto module tests must cover all code paths — security-critical code has no margin for untested branches.
