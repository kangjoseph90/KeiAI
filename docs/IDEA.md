...
- 스케일업 전략 (단계별 확장):
  ...
  - 3단계 (유료 VPS 이사): 텍스트 DB(`pb_data.db`)만으로도 용량이 부족해질 만큼 규모가 커지면, 유료 VPS로 DB 파일만 그대로 복사하여 이사. PocketBase 단일 바이너리 특성상 마이그레이션 비용 최소.
...
14. 서버 비용 충당 모델 (Sustainability)

- 핵심 철학: "로컬에서 쓰는 건 전부 무료. 클라우드 동기화에만 서버 비용이 발생하므로, 그 부분만 후원으로 충당한다."
- 기본 (무료):
  - 앱 다운로드 및 로컬 사용 무제한 (기기 수 무관).
...
- 쿼터 관리 (Quota Enforcement):
  ...
  - 서버 유저 레코드: quotaLimit (bytes), usedBytes (서버가 직접 카운트).
...
- 핵심 철학: 인프라 비용은 0원에 수렴하게 만들고, 유저의 신뢰는 100%로 끌어올린다.
...
- 핵심 원칙: 보안에 대한 신뢰는 코드로 증명하되, 서비스 운영에 필요한 인프라 로직은 보호한다.
...
- 비공개 영역 (Closed-Source): 서비스 운영 인프라 보호 구역.
  - 후원 및 쿼터 관리 로직: 동기화 쿼터 계산 및 업로드 차단 로직. (우회 방지)
  - Hub 커뮤니티 백엔드: 유저 공유 콘텐츠 서빙 로직.

19. 챗 파이프라인 (Stateless Generation Pipeline)
...
- 스크립트 실행 분리 (Placement):
  ...
  - `output`: 스트리밍 청크가 누적될 때 실시간 조작 (Generation Task 업데이트 및 최종 DB 저장용)
  - `display`: DB 원본은 건드리지 않고 브라우저 출력 직전에만 변경 (UI Message Component에서만 실행)
