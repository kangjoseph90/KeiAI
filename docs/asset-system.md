...
1. `AssetService.write()` normalizes supported media and applies convergent encryption.
...
Images, audio, and video share the same asset contract. `mimeType` determines rendering and provider mapping; dimensions preserve image and video layout when available.
...
Chat-owned inlays remain local-only for binary sync and server usage accounting.
