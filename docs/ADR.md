...
## 025: CharJS 다중 코드 블록 분할 (예정)

- 상태: 제안
- 맥락: ADR 024에서 인스턴스별 Mutex를 도입했으나, 하나의 캐릭터/모듈에 코드가 하나뿐이라 모든 핸들러(display, output, 이벤트 등)가 같은 Mutex를 공유한다. display 핸들러가 3초 타임아웃까지 실행되면 같은 인스턴스의 output 핸들러가 대기해야 한다.
- 문제: 관심사가 다른 핸들러(display 로직 vs LLM 후처리)가 같은 직렬화 큐에 있어 병목 발생.
- 대안 검토:
    - **Phase별 Mutex**: 핸들러 등록 단계(phase)별로 Mutex 분리. 구현 단순하지만, 동일 Phase 내 여러 핸들러는 여전히 직렬.
    - **핸들러별 Mutex**: 각 핸들러 함수에 독립 Mutex. QuickJS 컨텍스트 자체가 스레드 안전하지 않으므로 의미 없음 — 결국 컨텍스트 수준에서 직렬화 필요.
    - **다중 코드 블록 (슬롯)**: 캐릭터/모듈의 `charjs`를 `{ display: CodeBlock, output: CodeBlock, events: CodeBlock }`처럼 여러 슬롯으로 분할. 각 슬롯이 독립 QuickJS 인스턴스 + 독립 Mutex를 가짐.
- 결정 (예정): **다중 코드 블록 슬롯** 방식 채택 예정.
    - `CharJS` 타입을 `code: string`에서 `slots: Record<string, CodeBlock>`으로 변경.
    - 각 슬롯(예: `display`, `output`)이 독립 인스턴스(`CharJSInstance`)로 생성.
    - 캐시 키: `${ownerId}:${chatId}:${slotName}`.
    - 공유 상태는 ChatVar(`getVar`/`setVar`)를 DB 기반으로 전환하여 슬롯 간 통신 지원.
- 고려사항:
    - 메모리: 슬롯당 QuickJS 인스턴스 → 슬롯 수 × 메모리. TTL 캐시로 유휴 인스턴스 자동 정리.
    - API 호환성: 기존 단일 `code` 형식을 마이그레이션하는 방법 필요 (예: 빈 슬롯에 기본 코드 배치).
    - 개발자 경험: 슬롯 개념에 대한 학습 비용. 적절한 UI/에디터 지원 필요.
